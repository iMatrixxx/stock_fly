...
用法：
  python3 tools/forecast_card.py extract 复盘报告_2026-09-07.md
  python3 tools/forecast_card.py extract 复盘报告_2026-09-07.md --outdir <dir>
  python3 tools/forecast_card.py hint evidence_2026-09-07.json   # 打印候选清单

产物：forecast_<date>.json ——
...
def _parse_date(report_path: Path) -> str:
    m = _DATE_IN_NAME.search(report_path.name)
    if m:
        return m.group(1)
    raise ValueError(f"无法从报告文件名解析日期: {report_path.name}（用 --date 指定）")


def export_forecast_cards(date_str: str, report_md: Path, outdir: Path | None = None,
                          force: bool = False) -> dict | None:
    """把报告末尾预测卡区块冻结为 forecast_<date>.json。
    ...
    out = outdir / f"forecast_{date_str}.json"
...
def main(argv=None) -> None:
    ...
    p_extract.add_argument("report", help="复盘报告 Markdown 路径")
    p_extract.add_argument("--date", help="复盘日期 YYYY-MM-DD（默认从文件名解析）")
    p_extract.add_argument("--outdir", help="产物目录（默认仓库根目录）")
